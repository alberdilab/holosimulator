...
# ------------------------------------------------------------
# Main: genomics.json -> transcriptomics.json
# ------------------------------------------------------------

def genomics_to_transcriptomics_json(
    in_json: str | Path,
    out_json: str | Path,
    outdir: str | Path,
    genomes_subdir: str = "genomes",
    set_defaults: bool = True,
    max_contigs: Optional[int] = None,
    require_all: bool = False,
    # allocation choice + knobs
    distribute_reads: bool = True,
    allocation: str = "tpm",           # "tpm" | "length" | "uniform" | "zipf"
    allocation_params: dict | None = None,
    pairs_to_reads: int = 2,
    seed: Optional[int] = None,
) -> Path:
    """
    Augment genomics.json -> transcriptomics.json with contigs and (optionally) per-sample read arrays.

    Each contig becomes:
      {"id": ..., "length": ..., "gc": <0..1>, "reads": [r_s1, r_s2, ...]}
    Reads are positional arrays aligned to top-level 'samples'.

    Selection:
      allocation: pick a strategy from ALLOC_STRATEGIES
      allocation_params: dict of extra knobs for the chosen strategy (see the registry fns)
    """
    in_path = Path(in_json)
    out_path = Path(out_json)
    genomes_dir = Path(outdir) / genomes_subdir

    cfg = json.loads(in_path.read_text())
    genomes = cfg.get("genomes", [])
    samples: List[str] = cfg.get("samples", [])

    if not isinstance(samples, list) or not all(isinstance(s, str) for s in samples):
        raise ValueError("Top-level 'samples' must be a list of strings in the input JSON.")

    # pick allocator
    allocation = (allocation or "tpm").lower()
    if allocation not in ALLOC_STRATEGIES:
        raise ValueError(f"Unknown allocation '{allocation}'. Choose one of: {', '.join(ALLOC_STRATEGIES)}")
    alloc_fn = ALLOC_STRATEGIES[allocation]
    params = dict(allocation_params or {})

    # per-run RNG (per-sample derived RNGs)
    base_rng = np.random.default_rng(seed) if seed is not None else np.random.default_rng()

    for g in genomes:
        gid = g.get("id")
        if not gid:
            raise ValueError("Each genome must have an 'id'.")
        source = g.get("path", "")

        local_fa = _find_local_fasta(genomes_dir, gid, source)
        if not local_fa:
            if require_all:
                raise FileNotFoundError(f"Missing FASTA for {gid} under {genomes_dir} (from '{source}')")
            g["contigs"] = []
            g["n_contigs"] = 0
            g["reads_encoding"] = "array"
            g["samples_version"] = "|".join(samples)
            continue

        contigs = _scan_contigs(local_fa, max_contigs=max_contigs)

        # initialize per-contig reads arrays aligned to samples
        for c in contigs:
            c["reads"] = [0] * len(samples)

        if distribute_reads and contigs:
            ids = [c["id"] for c in contigs]
            clen = {c["id"]: int(c["length"]) for c in contigs}
            cgc  = {c["id"]: float(c.get("gc", 0.5)) for c in contigs}
            abund = g.get("abundances", {})

            for si, sname in enumerate(samples):
                total_pairs = int(float(abund.get(sname, 0)))
                total_reads = pairs_to_reads * total_pairs
                ...
                for c in contigs:
                    c["reads"][si] = int(alloc.get(c["id"], 0))

        g["contigs"] = contigs
        g["n_contigs"] = len(contigs)
        g["reads_encoding"] = "array"
        g["samples_version"] = "|".join(samples)

    if set_defaults:
        cfg.setdefault("granularity", "gene")
        cfg.setdefault("gene_allocation", allocation)
...
# ------------------------------------------------------------
# Alloc TSV materialization (exact FASTA-ID matching, includes zeros)
# ------------------------------------------------------------

def write_alloc_tsvs_from_transcriptomics_json(
    transcriptomics_json: str | Path,
    outdir: str | Path,
    genomes_subdir: str = "genomes",
) -> None:
    """
    Create alloc.tsv files (2 columns, no header: contig<TAB>reads) at:
      {outdir}/iss/{sample}/{gid}/alloc.tsv

    The TSV is written by enumerating the *actual* FASTA records (same file ISS will read)
    so IDs match exactly; any records absent in JSON get 0.
    """
    cfg = json.loads(Path(transcriptomics_json).read_text())
    genomes = cfg.get("genomes", [])
    samples = cfg.get("samples", [])
    genomes_dir = Path(outdir) / genomes_subdir

    # Build quick lookup: gid -> {contig_id -> [reads_per_sample]}
    per_gid: Dict[str, Dict[str, List[int]]] = {}
    for g in genomes:
        gid = g["id"]
        contigs = g.get("contigs", [])
        per_gid[gid] = {c["id"]: list(c.get("reads", [0] * len(samples))) for c in contigs}

    for g in genomes:
        gid = g["id"]
        local_fa = _find_local_fasta(genomes_dir, gid, g.get("path", ""))
        if not local_fa:
            # No FASTA -> still write empty alloc files so downstream can short-circuit cleanly
            for s in samples:
                ap = Path(outdir) / "iss" / s / gid / "alloc.tsv"
                ap.parent.mkdir(parents=True, exist_ok=True)
                ap.write_text("")  # empty
            continue

        fasta_ids = list(iter_fasta_ids(local_fa))
        contig_map = per_gid.get(gid, {})

        for si, s in enumerate(samples):
            ap = Path(outdir) / "iss" / s / gid / "alloc.tsv"
            ap.parent.mkdir(parents=True, exist_ok=True)
            with open(ap, "w") as w:
                for rid in fasta_ids:
                    arr = contig_map.get(rid)
                    n = int(arr[si]) if arr and si < len(arr) else 0
                    w.write(f"{rid}\t{n}\n")
